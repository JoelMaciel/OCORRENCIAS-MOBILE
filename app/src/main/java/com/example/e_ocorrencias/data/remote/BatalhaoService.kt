...
interface BatalhaoService {
    ...
    @GET("batalhoes")
    suspend fun getAllBatalhoes(@Query("page") page: Int): BatalhaoResponse

    @GET("batalhoes")
    suspend fun searchBatalhoes(@Query("nome") nome: String? = null): BatalhaoResponse

    @GET("batalhoes/{id}")
    suspend fun getBatalhaoById(@Path("id") id: String): Batalhao

    @POST("batalhoes")
    suspend fun createBatalhao(@Body batalhaoRequest: BatalhaoRequest): Batalhao
